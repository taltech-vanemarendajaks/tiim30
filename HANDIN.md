# TEAM 30 HANDIN


## PULL REQUESTS

https://github.com/taltech-vanemarendajaks/tiim30/pull/2

https://github.com/taltech-vanemarendajaks/tiim30/pull/5


## CONFLICT

branches test/PossibleConflict1 and branches test/PossibleConflict1 were created from same HEAD in develop
same row was modified. Merging first branch succeeded. Merging 2nd branch caused conflicts
Conflict was resolved by manuall changing file

  

## WORKED ON

Rain - raintuur

	Forked the repo
    Created team.md file and  reviewed pull request
  

Märten - MPuust

    Forked the repo to GitHub private repo
    Assigned team members
	Code refactor in InventoryController
	pull request 2

  

Eero - EeroNoormets

    Forked the rrepo to GitHub private repo
	Feature branch /docs/improve-onboarding-experience (Adding documentation for initial startup)
	Merging pull request2 from GitHub using Merge option
    Merging pull request5 using rebase option
	publishing HANDIN.md file
    Creating conflict in this file using 2 branches
  
Sigrid - SigridPihel

	Forked the repo to GitHub private repo
  
Katariina - Katariina-Parkja

	Forked the repo to GitHub private repo